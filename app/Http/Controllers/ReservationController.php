...
class ReservationController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $is_reserved = Reservation::where('user_id', '=', Auth::id())
        ->where('event_id', '=', $id)
        ->where('canceled_date', '=', null)
        ->latest()
        ->first();

        return view('event-detail', compact('event', 'reservable_people', 'is_reserved'));
    }
    ...
    public function reserve(Request $request)
    {
        ...
        if(is_null($reserved_people) || 
        $event->max_people >= $reserved_people->number_of_people + $request->reserved_people)
        {
            Reservation::create([
                'user_id' => Auth::id(),
                'event_id' => $request->id,
